...
5. **Create the FOMOD stub** at `build/releases/<Release Name>/fomod/` — `info.xml` and
   `ModuleConfig.xml`. Copy the shape from an existing release and keep it minimal; `fomod/` must sit
   at the **archive root**, which `build.ps1` handles. A single-plugin release needs only:
